...
class J :
  ...
  def attack(self, bouton_quit, bouton_1, bouton_2, bouton_3, adv, lst_joueur) :
    bouton_1.settexte(phrasesClass["J"]["attack"]["attackType"][0]), bouton_2.settexte(phrasesClass["J"]["attack"]["attackType"][1]), bouton_3.settexte(phrasesClass["J"]["attack"]["attackType"][2])
    self.boite_info()
    running = True
    selecAtta = False
    selecMA = False
    ...
    while running :
      ...
      for ev in pg.event.get():
        ...
        if ev.type == pg.MOUSEBUTTONDOWN:
          ...
          if bouton_quit.getwidth()[0] <= mouse[0] <= bouton_quit.getwidth()[1] and bouton_quit.getheight()[0] <= mouse[1] <= bouton_quit.getheight()[1] and bouton_quit.getstate() not in ["Down", "Dead"] :
            pg.quit()
            sys.exit()
      ...
      if selecMA : 
        ...
        if 100 - (35 - self.prec) < cp_crit <= 100 :
          if mana_uti != 0 :
            self.affiche_texte(phrasesClass["J"]["attack"]["fail"], lst_joueur, 2.5, adv)
          ...
          if atta_type == phrasesClass["J"]["attack"]["attackType"][0] :
            ...
            self.affiche_texte(phrasesClass["J"]["attack"]["crit"]["phy"][0] + str(degats) + phrasesClass["J"]["attack"]["crit"]["phy"][1], lst_joueur, 2.5, adv)
            pg.draw.rect(screen, (0, 0, 0), [0, 0, screen.get_width(), screen.get_height()])
            return(degats)

          elif atta_type == phrasesClass["J"]["attack"]["attackType"][1] :
            degats = self.force * 2 * 1.5 + int(mana_uti) * 2.5 * 1.5
            self.affiche_texte(phrasesClass["J"]["attack"]["crit"]["reinf"][0] + str(degats) + phrasesClass["J"]["attack"]["crit"]["reinf"][1], lst_joueur, 2.5, adv)
            self.mana -= int(mana_uti)
            pg.draw.rect(screen, (0, 0, 0), [0, 0, screen.get_width(), screen.get_height()])
            return(degats)
            ...
            self.affiche_texte(phrasesClass["J"]["attack"]["crit"]["mag"][0] + str(degats) + phrasesClass["J"]["attack"]["crit"]["mag"][1], lst_joueur, 2.5, adv)
            self.mana -= int(mana_uti)
            pg.draw.rect(screen, (0, 0, 0), [0, 0, screen.get_width(), screen.get_height()])
            return(degats)
          ...
          if atta_type == phrasesClass["J"]["attack"]["attackType"][0] :
            degats = self.force * 5
            self.affiche_texte(phrasesClass["J"]["attack"]["norm"]["phy"][0] + str(degats) + phrasesClass["J"]["attack"]["norm"]["phy"][1], lst_joueur, 2.5, adv)
            pg.draw.rect(screen, (0, 0, 0), [0, 0, screen.get_width(), screen.get_height()])
            return(degats)

          elif atta_type == phrasesClass["J"]["attack"]["attackType"][1] :
            degats = self.force * 2 + int(mana_uti) * 2.5
            self.affiche_texte(phrasesClass["J"]["attack"]["norm"]["reinf"][0] + str(degats) + phrasesClass["J"]["attack"]["norm"]["reinf"][1], lst_joueur, 2.5, adv)
            self.mana -= int(mana_uti)
            pg.draw.rect(screen, (0, 0, 0), [0, 0, screen.get_width(), screen.get_height()])
            return(degats)
            ...
            degats = int(mana_uti)
            self.affiche_texte(phrasesClass["J"]["attack"]["norm"]["mag"][0] + str(degats) + phrasesClass["J"]["attack"]["norm"]["mag"][1], lst_joueur, 2.5, adv)
            self.mana -= int(mana_uti)
            pg.draw.rect(screen, (0, 0, 0), [0, 0, screen.get_width(), screen.get_height()])
            return(degats)
  ...
  def defences(self, degats, bouton_quit, bouton_1, bouton_2, bouton_3, adv, lst_joueur) :
    bouton_1.settexte(phrasesClass["J"]["defense"]["defenseType"][0]), bouton_2.settexte(phrasesClass["J"]["defense"]["defenseType"][1]), bouton_3.settexte(phrasesClass["J"]["defense"]["defenseType"][2])
    running = True
    selecDef = False
    selecMD = False
    def_type = ""
    ...
    while running :
      ...
      if selecMD : 
        ...
        if 100 - (35 - self.prec) < def_crit <= 100 :
          if int(mana_uti) != 0 :
            self.affiche_texte(phrasesClass["J"]["defense"]["fail"], lst_joueur, 2.5, adv)
            self.mana -= int(mana_uti)
            self.pv -= degats
          ...
          if def_type == phrasesClass["J"]["defense"]["defenseType"][0] :
            ...
            self.affiche_texte(phrasesClass["J"]["defense"]["crit"]["phy"][0] + str(degats_def) + phrasesClass["J"]["defense"]["crit"]["phy"][1], lst_joueur, 2.5, adv)
            ...
            self.affiche_texte(phrasesClass["J"]["defense"]["crit"]["reinf"][0] + str(degats_def) + phrasesClass["J"]["defense"]["crit"]["reinf"][1], lst_joueur, 2.5, adv)
            self.mana -= int(mana_uti)
            ...
            self.affiche_texte(phrasesClass["J"]["defense"]["crit"]["mag"][0] + str(degats_def) + phrasesClass["J"]["defense"]["crit"]["mag"][1], lst_joueur, 2.5, adv)
            self.mana -= int(mana_uti)
          ...
          if def_type == phrasesClass["J"]["defense"]["defenseType"][0] :
            degats_def = self.defence * 2.5
            self.affiche_texte(phrasesClass["J"]["defense"]["norm"]["phy"][0] + str(degats_def) + phrasesClass["J"]["defense"]["norm"]["phy"][1], lst_joueur, 2.5, adv)
            ...
            self.affiche_texte(phrasesClass["J"]["defense"]["norm"]["reinf"][0] + str(degats_def) + phrasesClass["J"]["defense"]["norm"]["reinf"][1], lst_joueur, 2.5, adv)
            self.mana -= int(mana_uti)
            ...
            self.affiche_texte(phrasesClass["J"]["defense"]["norm"]["mag"][0] + str(degats_def) + phrasesClass["J"]["defense"]["norm"]["mag"][1], lst_joueur, 2.5, adv)
            self.mana -= int(mana_uti)
  ...
  def selec_atta(self, atta_type, selecAtta, bouton_1, bouton_2, bouton_3) :
    ...
    if not selecAtta :
      if atta_type == "" :
        screen.blit(atkfont.render(phrasesClass["J"]["selection"]["atk"][0], True, (255, 255, 255)), (int(screen.get_width()/5), 15))
      else :
        screen.blit(atkfont.render(phrasesClass["J"]["selection"]["atk"][1] + str(atta_type), True, (255, 255, 255)), (int(screen.get_width()/5), 15))
        selecAtta = True
        return selecAtta
  ...
  def selec_def(self, def_type, selecDef, bouton_1, bouton_2, bouton_3) :
    ...
    if not selecDef :
      if def_type == "" :
        screen.blit(atkfont.render(phrasesClass["J"]["selection"]["def"][0], True, (255, 255, 255)), (int(screen.get_width()/5), 15))
      else :
        screen.blit(atkfont.render(phrasesClass["J"]["selection"]["def"][1] + str(def_type), True, (255, 255, 255)), (int(screen.get_width()/5), 15))
        selecDef = True
        return selecDef
  
  def mana_atta(self, mana_uti, selecMD) :
    if not selecMD :
        screen.blit(atkfont.render(phrasesClass["J"]["selection"]["mana"][0] + str(mana_uti), True, (255, 255, 255)), (int(screen.get_width()/5), 15))
    else :
        screen.blit(atkfont.render(phrasesClass["J"]["selection"]["mana"][1][0] + str(mana_uti) + phrasesClass["J"]["selection"]["mana"][1][1], True, (255, 255, 255)), (int(screen.get_width()/5), 15))
        selecMD = True
        return selecMD
  ...
  def boite_info(self) : 
    ...
    retirer = vsmallfont.render(str(self.pv)+phrasesClass["J"]["infoBox"]["pv"], 1, (255, 255, 255)).get_rect()[2]
    pos_0 = 165 + retirer
    pos_1 = 116
    pos_2 = 218
    screen.blit(vsmallfont.render(self.nom, 1, (255, 255, 255)), [25, 30 + (120 * (self.id - 1) + 20 * (self.id - 1)), 50, 25])
    screen.blit(vsmallfont.render(str(self.pv)+phrasesClass["J"]["infoBox"]["pv"], 1, (255, 255, 255)), [pos_0 - retirer, 30 + (120 * (self.id - 1) + 20 * (self.id - 1)), retirer, 25])
    screen.blit(vsmallfont.render(phrasesClass["J"]["infoBox"]["stats"], 1, (255, 255, 255)), [25, 55 + (120 * (self.id - 1) + 20 * (self.id - 1)), 50, 25])
    screen.blit(vsmallfont.render(phrasesClass["J"]["infoBox"]["strength"], 1, (255, 255, 255)), [25, 75 + (120 * (self.id - 1) + 20 * (self.id - 1)), 50, 25])
    screen.blit(vsmallfont.render(phrasesClass["J"]["infoBox"]["defense"], 1, (255, 255, 255)), [127, 75 + (120 * (self.id - 1) + 20 * (self.id - 1)), 50, 25])
    screen.blit(vsmallfont.render(phrasesClass["J"]["infoBox"]["mana"], 1, (255, 255, 255)), [25, 95 + (120 * (self.id - 1) + 20 * (self.id - 1)), 50, 25])
    screen.blit(vsmallfont.render(phrasesClass["J"]["infoBox"]["precision"], 1, (255, 255, 255)), [127, 95 + (120 * (self.id - 1) + 20 * (self.id - 1)), 50, 25])
  ...
  def info_attaque(self) :
    msg = smallfont.render(phrasesClass["J"]["info"]["attack"], 1, (255, 255, 255))
    text_rect = msg.get_rect(center = (screen.get_width() / 2, screen.get_height() / 2 - 50))
    screen.blit(msg, text_rect)

  def info_defence(self) :
    msg = smallfont.render(phrasesClass["J"]["info"]["defense"], 1, (255, 255, 255))
    text_rect = msg.get_rect(center = (screen.get_width() / 2, screen.get_height() / 2 + 50))
    screen.blit(msg, text_rect)
  
  def affiche_texte(self,texte,liste_joueur,temps,adv) : 
    bouton_quit = Bouton(screen.get_width() - 155, screen.get_width() - 15, screen.get_height() - 55, screen.get_height() - 15, phrasesClass["Bouton"]["Quit"], font, (255, 255, 255), "")
    duree = temps * 60
    while duree > 0 :
      clock.tick(60)
      pg.draw.rect(screen, (0, 0, 0), [0, 0, screen.get_width(), screen.get_height()])
...
class Banshee :
  ...
  def __init__(self, liste_joueur) :
    ...
    if len(liste_joueur) > 1 :
      self.pv = self.pvbase = int(2500 * (len(liste_joueur) * (1 + (len(liste_joueur) - 1 ) * 0.25)))
  ...
  def attack(self, joueur_att, liste_joueur) :
    ...
    pg.draw.rect(screen, (0, 0, 0), [0, 0, screen.get_width(), screen.get_height()])
    self.affiche_texte(phrasesClass["banshee"]["attack"]["prepare"], liste_joueur, 3)
    pg.draw.rect(screen, (0, 0, 0), [0, 0, screen.get_width(), screen.get_height()])
    
    if 0 < att_banshee <= 60 :
      degats = 250
      self.affiche_texte(phrasesClass["banshee"]["attack"]["norm"]["bas"][0] + str(joueur_att.getnom()) + phrasesClass["banshee"]["attack"]["norm"]["bas"][1] + str(degats) + phrasesClass["banshee"]["attack"]["norm"]["bas"][2], liste_joueur, 4)
      self.mana -= 150
      return degats, att_banshee

    if 60 < att_banshee <= 95 :
      degats = 500
      self.affiche_texte(phrasesClass["banshee"]["attack"]["norm"]["moy"][0] + str(joueur_att.getnom()) + phrasesClass["banshee"]["attack"]["norm"]["moy"][1] + str(degats) + phrasesClass["banshee"]["attack"]["norm"]["moy"][2], liste_joueur, 4)
      self.mana -= 375
      return degats, att_banshee

    if 95 < att_banshee <= 100 and self.pv <= 1500 :
      degats = self.pv
      self.affiche_texte(phrasesClass["banshee"]["attack"]["norm"]["haut"][0] + str(degats)+ phrasesClass["banshee"]["attack"]["norm"]["haut"][1], liste_joueur, 4)
      self.pv = 0
      self.mana -= 500
    ...
    if 95 < att_banshee <= 100 :
      degats = 750
      self.affiche_texte(phrasesClass["banshee"]["attack"]["norm"]["noHaut"][0] + str(degats)+ phrasesClass["banshee"]["attack"]["norm"]["noHaut"][1], liste_joueur, 4)
      self.pv -= 125
      self.mana -= 375
  ...
  def defences(self, degats, liste_joueur) :
    def_crit = random.randint(1,100)
    if 100 - (35 - self.prec) < def_crit <= 100 :
      self.affiche_texte(phrasesClass["banshee"]["defense"]["fail"][0] + str(degats + 50)+ phrasesClass["banshee"]["defense"]["fail"][1], liste_joueur, 4)
      self.pv -= degats + 50

    elif degats != 0 :
      self.affiche_texte(phrasesClass["banshee"]["defense"]["succes"][0] + str(degats - 50)+ phrasesClass["banshee"]["defense"]["succes"][1], liste_joueur, 4)
      self.pv -= degats - 50
  ...
  def boite_info(self) : 
    retirer = vsmallfont.render(str(self.pv)+phrasesClass["banshee"]["infoBox"]["pv"], 1, (255, 255, 255)).get_rect()[2]
    ...
    screen.blit(vsmallfont.render(str(self.pv)+phrasesClass["banshee"]["infoBox"]["pv"], 1, (255, 255, 255)), [pos_0 - 79 - retirer, 30, retirer, 25])
    screen.blit(vsmallfont.render(phrasesClass["banshee"]["infoBox"]["stats"], 1, (255, 255, 255)), [pos_1 - 220, 55, 50, 25])
    screen.blit(vsmallfont.render(phrasesClass["banshee"]["infoBox"]["strength"], 1, (255, 255, 255)), [pos_1 - 220, 75, 50, 25])
    screen.blit(vsmallfont.render(phrasesClass["banshee"]["infoBox"]["defense"], 1, (255, 255, 255)), [pos_1 - 118, 75, 50, 25])
    screen.blit(vsmallfont.render(phrasesClass["banshee"]["infoBox"]["mana"], 1, (255, 255, 255)), [pos_1 - 220, 95, 50, 25])
    screen.blit(vsmallfont.render(phrasesClass["banshee"]["infoBox"]["precision"], 1, (255, 255, 255)), [pos_1 - 118, 95, 50, 25])
    retirer = vsmallfont.render(str(self.force), 1, (255, 255, 255)).get_rect()[2]
...
class Night_walker :
  def __init__(self,liste_joueur) :
    ...
    self.ombre = 0
    self.turn = True
    self.BarreDeVie = BarreDeVie(400, self.nom, self.pv, self.pvbase, self.pvbase/4)
...
class Histoire :
  def __init__(self,texte,font2) : 
    self.bouton_quit = Bouton(screen.get_width() - 155, screen.get_width() - 15, screen.get_height() - 55, screen.get_height() - 15, phrasesClass["Bouton"]["Quit"], font, (255, 255, 255), "")
    self.font = font2
  ...
  def affiche_histoire(self) :
    ...
    while game_over :
      ...
      for ev in pg.event.get():
        ...
        if ev.type == pg.MOUSEBUTTONDOWN:
          mouse = pg.mouse.get_pos()
          if self.bouton_quit.getwidth()[0] <= mouse[0] <= self.bouton_quit.getwidth()[1] and self.bouton_quit.getheight()[0] <= mouse[1] <= self.bouton_quit.getheight()[1] and self.bouton_quit.getstate() not in ["Down", "Dead"] :
            pg.quit()
            game_over = False
            sys.exit()
...
class SliderS : 
  def __init__(self, x, y, length, height, max) :
    ...
    self.max = max
    self.current = 1
    self.slider = Slider(screen, x, y, length, height, min=1, max=max, step=1, initial=1, colour=(150, 20, 200))
    self.output = TextBox(screen, 900, y-16, 100, 100, fontSize=30, textColour=(240, 240, 240), colour=(0, 0, 0))
    self.textMax = TextBox(screen, 750, y-16, 150, 100, fontSize=30, textColour=(240, 240, 240), colour=(0, 0, 0))
    self.output.disable()
  ...
  def update(self) : 
    ...
    self.slider = Slider(screen, self.x, self.y, self.length, self.height, min=1, max=self.max, step=1, initial=int(self.current))
    
  def ChangeColor(self, color) :
    if color == "Red" :
      self.current = self.getValue()
      self.slider = Slider(screen, self.x, self.y, self.length, self.height, min=1, max=self.max, step=1, initial=int(self.current), colour=(150, 0, 0))
    else : 
      self.current = self.getValue()
      self.slider = Slider(screen, self.x, self.y, self.length, self.height, min=1, max=self.max, step=1, initial=int(self.current), colour=(150, 20, 200))
  
  def reset(self) : 
    self.slider = Slider(screen, self.x, self.y, self.length, self.height, min=1, max=self.basemax, step=1, initial=1, colour=(150, 20, 200))
...
class GestionSlider :
  def __init__(self, sliderStr, sliderDef, sliderPrec):
    ...
    self.CurColor = "White"
    self.tot = TextBox(screen, 950, 100, 200, 60, fontSize=30, textColour=(240, 240, 240), colour=(0, 0, 0))
  ...
  def update(self) : 
    tot = int(self.Str.getValue()) + int(self.Def.getValue()) + int(self.prec.getValue())
  ...
  def ChangeColor(self, color) : 
    if color == "Red" : 
      self.tot = TextBox(screen, 950, 100, 200, 60, fontSize=30, textColour=(200, 0, 0), colour=(0, 0, 0))
  ...
  def getTot(self) : 
    return int(self.Str.getValue()) + int(self.Def.getValue()) + int(self.prec.getValue())
...
class BarreDeVie : 
  ...
  def AfficheBar(self) : 
    ...
    progressBar = ProgressBar(screen, width/4, 50, width/2, 18, lambda: (self.pv / self.pvbase), curved=True, completedColour=completedColour)
...
def Jouer():
  ...
  bouton_quit = Bouton(screen.get_width() - 155, screen.get_width() - 15, screen.get_height() - 55, screen.get_height() - 15, phrasesClass["Bouton"]["Quit"], font, (255, 255, 255), "")
  bouton_compris = Bouton(screen.get_width() - 155, screen.get_width() - 15, screen.get_height() - 120, screen.get_height() - 80, phrasesClass["Bouton"]["Rules"], atkfont, (255, 255, 255), "")
  bouton_1 = Bouton(15, 155, screen.get_height() - 55, screen.get_height() - 15, phrasesClass["Bouton"]["BTN1"], atkfont, (255, 255, 255), "")
  bouton_2 = Bouton(170, 310, screen.get_height() - 55, screen.get_height() - 15, phrasesClass["Bouton"]["BTN2"], atkfont, (255, 255, 255), "")
  bouton_3 = Bouton(325, 465, screen.get_height() - 55, screen.get_height() - 15, phrasesClass["Bouton"]["BTN3"], atkfont, (255, 255, 255), "")
  bouton_4 = Bouton(480, 620, screen.get_height() - 55, screen.get_height() - 15, phrasesClass["Bouton"]["BTN4"], atkfont, (255, 255, 255), "")
  sliders = GestionSlider(SliderS(100, 200, 600, 20, 90), SliderS(100, 300, 600, 20, 90), SliderS(100, 400, 600, 20, 25))
  ...
  while game_over == 0 :
    ...
    for ev in pg.event.get():
      ...
      if ev.type == pg.MOUSEBUTTONDOWN:
        ...
        elif bouton_1.getwidth()[0] <= mouse[0] <= bouton_1.getwidth()[1] and bouton_1.getheight()[0] <= mouse[1] <= bouton_1.getheight()[1] and bouton_1.getstate() == "" :
            if not selecPerso : 
              perso = 1
    ...
    if not selecPerso :
      ...
      if perso > 0 :
        ...
        if (not selecStat) and selecMana : 
          selecStat = nbstats(stats, force, Def, prec, selecStat, bouton_1, sliders)
          if selecStat : 
            pg.display.flip()
            pg.time.delay(1000)
          pg.display.flip()
              ...
              for ev in pg.event.get() : 
                ...
                if ev.type == pg.MOUSEBUTTONDOWN : 
                  ...
                  if bouton_quit.getwidth()[0] <= mouse[0] <= bouton_quit.getwidth()[1] and bouton_quit.getheight()[0] <= mouse[1] <= bouton_quit.getheight()[1] and bouton_quit.getstate() not in ["Down", "Dead"] :
                    ...
                    game_over = 3
                    sys.exit()
                  elif bouton_compris.getwidth()[0] <= mouse[0] <= bouton_compris.getwidth()[1] and bouton_compris.getheight()[0] <= mouse[1] <= bouton_compris.getheight()[1] and bouton_compris.getstate() not in ["Down", "Dead"] :
                    stop = 1
              pg.draw.rect(screen, (0, 0, 0), [0, 0, screen.get_width(), screen.get_height()])
              J1.info_attaque()
              J1.info_defence()
...
bouton_Jouer = Bouton(screen.get_width() / 2 - 100, screen.get_width() / 2 + 100, screen.get_height() / 2 - 25, screen.get_height() / 2 + 25, phrasesClass["Bouton"]["Play"], font, (0, 200, 0), "")
bouton_quit = Bouton(screen.get_width() / 2 - 100, screen.get_width() / 2 + 100, screen.get_height() - 165, screen.get_height() - 115, phrasesClass["Bouton"]["Quit"], font, (255, 255, 255), "")
